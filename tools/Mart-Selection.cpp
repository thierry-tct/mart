...
// Remove the option we don't want to display in help
  ...
  assert(llvm::sys::fs::exists(mutantInfoJsonfile) && "Error: the topdir given do not exist!");
  
  if (mutantInfoJsonfile.empty()) { //try to get it from martOutTopDir
    mutantInfoJsonfile = martOutTopDir + "/" + mutantsInfosFileName;
    //make sure it exists
    assert (llvm::sys::fs::exists(mutantInfoJsonfile) && "The specified topdir do not contain mutantInfofile and none was specified");
  } else {
    assert (llvm::sys::fs::exists(mutantInfoJsonfile) && "The specified mutantInfofile do not exist");
  }
  if (inputIRfile.empty()) {  //try to get it from martOutTopDir: end with .bc and is neither .WM.bc or .preTCE.MetaMu.bc, or .MetaMu.bc
    ...
    while (!ec) {
      ...
      if (fpath.endswith(commonIRSuffix)) {
        if(!fpath.endswith(commonIRSuffix) && !fpath.endswith(commonIRSuffix) && !fpath.endswith(commonIRSuffix)) {
    ...
    assert (llvm::sys::fs::exists(inputIRfile) && "The specified input BC file do not exist");
  }
