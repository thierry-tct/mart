...
class MutationScope
{
  ...
  public:
    ...
    {
        assert (isInitialized() && "checking function in uninitialized scope object");
        if (mutateAllFuncs)
            return true;
        else
...
struct MatchStmtIR
{
    ...
    void dumpIRs() const {llvm::errs()<<"\n"; for (auto *x: toMatchIRs) x->dump();}
...
{
    /// This struct represent the toMatchMutant (Irs BB for the mutants and the corresponding BB in original)
    struct MutantStmtIR
    {
        ...
        void dumpIRs() const {llvm::errs()<<"\n"; for (auto *x: toMatchIRsMutClone) x->dump();}
        ...
        /**
         * \brief insert an IR into this mutantstmt, THIS MUTANT MUST HAVE BEEN CLONED FOM THE ORIGINAL, not set with setToEmptyOrFixedStmtOf()..
         */
        inline void insertIRAt (unsigned ind, llvm::Value *irVal)      //TODO: fix for the case of empty middle block
        {
            ...
            {   //last position of toMatchIRsMutClone, which cannot be empty since it is cloned from original
                assert (origBBToMutBB.size() >= 1 && "must have at least one BB here (since it must have been cloned)");
                llvm::BasicBlock *bb;
        ...
        /**
         * \brief insert IRs into this mutantstmt, THIS MUTANT MUST HAVE BEEN CLONED FOM THE ORIGINAL not set with setToEmptyOrFixedStmtOf()..
         */
        ...
        inline void eraseIRAt (unsigned ind) 
        {
            if (!llvm::isa<llvm::Constant>(toMatchIRsMutClone[ind]))
            {
                ...
                {
                    llvm::errs() << "\nError: Deleting IR while its use is not empty. Please report Bug (MART)!\n\n";
        ...
        void setToEmptyOrFixedStmtOf(MatchStmtIR const &toMatch, ModuleUserInfos const &MI, llvm::SmallVector<llvm::Instruction *, 1> *fixedinsts = nullptr)
        {
            ...
            for (auto i=0;i<toMatch.getNumBB();i++)
            {
                ...
                origBBToMutBB[toMatch.getBBAt(i)] = std::vector<llvm::BasicBlock *>({bb});
                if (fixedinsts)
                {
...
struct MutantInfoList  
{
    struct MutantInfo
    {
        ...
        MutantInfo (MutantIDType mutant_id, std::string const &type, std::string const &funcName, std::vector<unsigned> const &irPos, std::string const &srcLoc)
                        :id(mutant_id), typeName(type), locFuncName(funcName), irLeveLocInFunc(irPos), srcLevelLoc(srcLoc) {}

        MutantInfo (MutantIDType mid, std::vector<llvm::Value *> const &toMatch, std::string const &mName, std::vector<unsigned> const &relpos, llvm::Function *curFunc, std::vector<unsigned> const & absPos)//:
            //typeName (mName)
        {
            id = mid;
            typeName = mName;
  ...
  private:  
    ...
    void internalAdd (MutantIDType mutant_id, std::string const &type, std::string const &funcName, std::vector<unsigned> const &irPos, std::string const &srcLoc)
    {
     ...
     */
    bool wasAdded (MutantIDType mid) const {return (containedMutsIDs.count(mid) > 0);}
  ...
  public:  
     ...
     */
    void add (MutantIDType mid, std::vector<llvm::Value *> const &toMatch, std::string const &mName, \
    ...
    const std::string &getMutantTypeName(MutantIDType mutant_id) const {return mutants[mutant_id-1].typeName;}
    const std::string &getMutantFunction(MutantIDType mutant_id) const {return mutants[mutant_id-1].locFuncName;}
    const std::vector<unsigned> &getMutantIrPosInFunction(MutantIDType mutant_id) const {return mutants[mutant_id-1].irLeveLocInFunc;}
    const std::string &getMutantSourceLoc(MutantIDType mutant_id) const {return mutants[mutant_id-1].srcLevelLoc;}
