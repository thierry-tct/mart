...
/*class MutantNode : public Node<MutantDependenceGraph, MutantIDType, MutantNode>
{
public:
    MutantNode(MutantIDType id)
        :dg::Node<MutantDependenceGraph, MutantIDType, MutantNode>(id){}
...
class MutantDependenceGraph //: public DependenceGraph<MutantNode>
{
    struct MutantDepends
    {
        ...
        std::unordered_set<MutantIDType> tieDependents;   // Share IR (mutants on same statement)
    };
  private:
    std::vector<MutantDepends> mutantDGraphData;  //Adjacent List
    std::unordered_map<MutantIDType, std::unordered_set<llvm::Value const *>> mutant2IRset;
    std::unordered_map<llvm::Value const *, std::unordered_set<MutantIDType>> IR2mutantset;
  ...
  public:
    MutantDependenceGraph(MutantIDType nMuts) {mutantDGraphData.resize(nMuts + 1);}
    
    std::unordered_map<llvm::Value const *, std::unordered_set<MutantIDType>> const &getIR2mutantsetMap() const {return IR2mutantset;}
    std::unordered_set<llvm::Value const *> &getIRsOfMut (MutantIDType id) {return mutant2IRset.at(id);}
    bool isBuilt () {return (! mutant2IRset.empty());}
    
    MutantIDType getMutantsNumber() {return mutantDGraphData.size()-1;}
    
    const std::unordered_set<MutantIDType> &getOutDataDependents(MutantIDType mutant_id) {return mutantDGraphData[mutant_id].outDataDependents;}
    const std::unordered_set<MutantIDType> &getInDataDependents(MutantIDType mutant_id) {return mutantDGraphData[mutant_id].inDataDependents;}
    const std::unordered_set<MutantIDType> &getOutCtrlDependents(MutantIDType mutant_id) {return mutantDGraphData[mutant_id].outCtrlDependents;}
    const std::unordered_set<MutantIDType> &getInCtrlDependents(MutantIDType mutant_id) {return mutantDGraphData[mutant_id].inCtrlDependents;}
    
    const std::unordered_set<MutantIDType> &getTieDependents(MutantIDType mutant_id) {return mutantDGraphData[mutant_id].tieDependents;}
...
class MutantSelection
{
  private:
    ...
    MutantIDType pickMutant (std::unordered_set<MutantIDType> const &candidates, std::vector<double> const &scores);
  ...
  public:
    ...
    void smartSelectMutants (std::vector<MutantIDType> &selectedMutants, std::vector<double> &selectedScores);
    void randomMutants (std::vector<MutantIDType> &spreadSelectedMutants, std::vector<MutantIDType> &dummySelectedMutants, unsigned long number);
    void randomSDLMutants (std::vector<MutantIDType> &selectedMutants, unsigned long number);   //only statement deletion mutants
};
