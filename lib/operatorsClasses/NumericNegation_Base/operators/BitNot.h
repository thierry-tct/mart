#ifndef __KLEE_SEMU_GENMU_operatorClasses__BitNot__
#define __KLEE_SEMU_GENMU_operatorClasses__BitNot__
 ...
 *  
 * \brief     Generic abstract base class for all non-pointer integer bitnot negation mutation operator.
...
class BitNot: public NumericArithNegation_Base
{
        ...
        if (propNeg->getOpcode() == llvm::Instruction::Xor)
        {
            if (llvm::ConstantInt *theConst = llvm::dyn_cast<llvm::ConstantInt>(propNeg->getOperand(0)))
            {
                if (theConst->equalsInt(-1)) 
                    ...
                    if (llvm::isa<llvm::ConstantInt>(propNeg->getOperand(1)) && llvm::dyn_cast<llvm::ConstantInt>(propNeg->getOperand(1))->equalsInt(-1))
                        oprdId = 0;
            }
            else
            {
                if (llvm::ConstantInt *theConst = llvm::dyn_cast<llvm::ConstantInt>(propNeg->getOperand(1)))
                    if (theConst->equalsInt(-1))
                        oprdId = 0;
            }
        }
  ...
  public:
    llvm::Value * createReplacement (llvm::Value * oprd1_addrOprd, llvm::Value * oprd2_intValOprd, std::vector<llvm::Value *> &replacement, ModuleUserInfos const &MI)
    {
        ...
        llvm::Value *bitnot = builder.CreateXor(oprd1_addrOprd, llvm::Constant::getAllOnesValue(oprd1_addrOprd->getType())); //llvm::ConstantInt::get(oprd1_addrOprd->getType(), -1));
...
#endif //__KLEE_SEMU_GENMU_operatorClasses__BitNot__
