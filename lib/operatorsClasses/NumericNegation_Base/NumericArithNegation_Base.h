#ifndef __KLEE_SEMU_GENMU_operatorClasses__NumericArithNegation_Base__
#define __KLEE_SEMU_GENMU_operatorClasses__NumericArithNegation_Base__
...
class NumericArithNegation_Base: public GenericMuOpBase
{
  protected:
    /**
     * \brief This method is to be implemented by each numeric arithmetic negation matching-mutating operator
     * \detail we want to know if the negation is matched and return the operand index of the negated value in the binop
     * @return the operand number in the binary operation if it match with the corresponding negation, else -1.
     */
...
#endif //__KLEE_SEMU_GENMU_operatorClasses__NumericArithNegation_Base__
