 ...
 * \todo      Rework the design to have 'and' and 'or' separated and make use of matchIRs and prepareClone
 * \todo      Add support for a&&b --> b&&a, as well as a||b --> b||a, a&&b --> b||a ...
    ...
    void matchAndReplace (std::vector<llvm::Value *> const &toMatch, llvmMutationOp const &mutationOp, MutantsOfStmt &resultMuts, bool &isDeleted, ModuleUserInfos const &MI)
    {
        ...
        for (auto *val:toMatch)
        {
            ...
            if (llvm::BranchInst *br = llvm::dyn_cast<llvm::BranchInst>(val))
            {
                ...
                /// No need to checktype because logical is always on boolean
                //if (!(checkCPTypeInIR (mutationOp.getCPType(1), searchedBr->getCondition())) 
                //    continue;
                    ...
                    else
                    {
                        ...
                        {   //directly replace
                            ...
                            switch (repl.getExpElemKey())
                            {
                                case mAND:
                                {
                                    assert (mutationOp.getMatchOp() == mOR && "only 'or' can change to 'and' here");
                                ...
                                case mOR:
                                {
                                    assert (mutationOp.getMatchOp() == mAND && "only 'and' can change to 'or' here");
