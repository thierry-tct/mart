#ifndef __KLEE_SEMU_GENMU_operatorClasses__FPLeftDec__
#define __KLEE_SEMU_GENMU_operatorClasses__FPLeftDec__
...
class FPLeftDec : public NumericArithIncDec_Base {
protected:
  /**
...
public:
  llvm::Value *createReplacement(llvm::Value *oprd1_addrOprd,
    ...
    std::vector<llvm::Value *> seenCasts; // cast in the opposite order of uses:
                                          // { int(float(char(x)) -> [char,
                                          // float, int] }
...
#endif //__KLEE_SEMU_GENMU_operatorClasses__FPLeftDec__
