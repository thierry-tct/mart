#ifndef __KLEE_SEMU_GENMU_operatorClasses__NumericArithIncDec_Base__
#define __KLEE_SEMU_GENMU_operatorClasses__NumericArithIncDec_Base__
 ...
 *  
 * \brief     Generic abstract base class for all non-pointer arithmetic increment/decrement mutation operator.
...
class NumericArithIncDec_Base: public GenericMuOpBase
{
     ...
     * @return true if the parameters are matching the requirement for the class (leftinc (x+1), rightinc(x-1)). else false
     * @param modif is the operation that add or substract 1
     * @param constpart is the constant representing either 1 or -1 
     * @param notloadat01 is either 0 or 1 and says that the operand of modif at 0 or 1 is not the load instruction
     */
    inline virtual bool isMyAdd1Sub1(llvm::BinaryOperator *modif, llvm::Constant *constpart, unsigned notloadat01) = 0;
    
    /**
     * \brief This method is to be implemented by each numeric increment decrement matching-mutating operator
     * @return true if the increment or decrement is 'left' (like ++i, --i). otherwise false (like i++, i--)
     */
...
#endif //__KLEE_SEMU_GENMU_operatorClasses__NumericArithIncDec_Base__
