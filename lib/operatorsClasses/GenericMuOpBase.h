 ...
 * \details   This abstract class define the minimal interface to be defined by all mutation operators.
...
{
  private:
    struct MatchUseful *additionalMU;   //When there are many matches at a pos, linklist then using this
    struct MatchUseful *curLast;
    
    /// \brief This vector is the list of pairs representing the IR used to make high level operands
    /// \brief the first element of the pair is the position (pos) in 'toMatch'. The second is a negative int if the value of the IR is considered
    /// \brief else, it represents the Operand index of the IR (toMatch[pos]). 
    std::vector<std::pair<unsigned/*pos*/, int/*oprd index*/>> highLevelOprdsSources;
  ...
  public:  
    MatchUseful() :additionalMU(nullptr)
    {
        clearAll();
    }
...
/**
 * This structure - 'DoReplaceUseful' - represent the data fields returned by the
...
struct DoReplaceUseful
{
  ...
  public:  
    ...
    inline void appendHLOprds (llvm::Value *oprdV) {highLevelOprds.push_back(oprdV);}
    inline llvm::Value * getHLOprdOrNull (int ind) {return (ind < highLevelOprds.size() ?highLevelOprds.at(ind): nullptr);}
    
    //inline void setHLReturningIR (llvm::Value *val) {highLevelReturningIR = val;}
    //inline llvm::Value * getHLReturningIR () {return highLevelReturningIR;}
    
    //inline void appendOrigRelevantIRPos (int pos) {origRelevantIRPos.push_back(pos);}
    inline void setOrigRelevantIRPos (std::vector<unsigned> const &posVect) {origRelevantIRPos = &posVect;}
    inline int getOrigRelevantIRPosOf (int posInList) {return origRelevantIRPos->at(posInList);}
    inline const std::vector<unsigned> & getOrigRelevantIRPos () {return *origRelevantIRPos;}
...
class GenericMuOpBase
{
  public:
     ...
     * @param pos is the position (starting from 0) of the IR instruction starting point of the match temptative.
     * @param MU is the structure where the information about the matching are store. It can be used later to prepare cloneIRs (@see prepareCloneIRs()).
     ...
     * @param toMatch is the statement in which we want to match the candidate pattern to muatate.
     * @param pos is the position (starting from 0) of the IR instruction starting point of the match temptative.
     * @param MU is the structure where the information about the matching were store (@see matchIRs()).
     ...
     * @param lh_addrOprdptr is the high level left hand operand for non-pointer operation, and pointer (address) operand for operation on pointers. 
     * @param rh_addrOprdptr is the high level right hand operand for non-pointer operation, and non-pointer (non-address) operand for operation on pointer.
     * @param replacement is the list of IRs representing the created operation
     * @param MI is the parameter containing the global information.
     */
    virtual llvm::Value * createReplacement (llvm::Value * oprd1_addrOprd, llvm::Value * oprd2_intValOprd, std::vector<llvm::Value *> &replacement, ModuleUserInfos const &MI) = 0;
    
    /**  A virtual member.
     * \brief Match the IR pattern and mutate. This is entry-point method called during mutation. Reimplement this as needed.
     * \detail The method take a statement, a code pattern type to match (Ex: Add 2 integers) as well as the muatant code pattern and generatemutated clones of the statement.
     * @param toMatch is the statement in which we want to match the candidate pattern to muatate.
  ...
  protected:
     ...
     * \detail Use this when matching IR with code parts to mutate, to match each oprd in IR with expected CPType
    ...
    /**
     * \brief This method obtain a constant numeric from a string contained in a static structure of @see llvmMutationOp.
     * @param type is the type of the constant to return (integer - i64, i32... or FloatingPoint...).
     * @param posConstValueMap_POS is the position of the string to convert into constant. This assumes that the position is within bounds and the string represent a constant. Otherwise Error.
     * @return the the constant obtained from the string.
     * \todo Add support for integer constant from string representing integer in radix 16 and 8 (other than radix 10)
     */
    llvm::Constant * createIfConst (llvm::Type *type, unsigned posConstValueMap_POS)
    {
    ...
    /**
     * \brief The method applies the reverse of a "cast" instruction. Ex: passing IntToPtr will return PtrToInt.
     ...
     * @param destTy is the destination subtype type within the general type returned by the reverse cast.  
     */
     ...
     */
    inline enum ExpElemKeys getGenericDeleterCode () 
    {
        return mDELSTMT;
    }
    ...
    /**
     * \brief Check whether two values of the same type are constant and of same values
     *@return true if the two values are constant and have same value, false otherwise
     */
     ...
     *                                      Here the corresponding operand index is also specified ('retIntoOprdIndex'). Note that 'returningIR' must be null in this case.
     * @param toMatch is the original statement that have been matched (still attached to the original Module).
     * @param resultMuts is the list of all generated mutants of the original statement 'toMatch'. Note: non attached to original Module.
                ...
                std::vector<unsigned> const &relevantPosInToMatch, ModuleUserInfos const &MI, int returnIntoIRPos=-1, int retIntoOprdIndex=-1)
    {
        ...
        //assert ((repl.getExpElemKey() != mKEEP_ONE_OPRD || (repl.getOprdIndexList().size()==1 && repl.getOprdIndexList()[0] < 2)) && "Error in the replacor 'mKEEP_ONE_OPRD'");
