#ifndef __KLEE_SEMU_GENMU_operatorClasses__FunctionCall__
#define __KLEE_SEMU_GENMU_operatorClasses__FunctionCall__
...
class FunctionCall: public MatchOnly_Base
{
  private:
    ...
    void generateCombinations (llvm::CallInst *call, unsigned shuffle_size/*Number of params involved in each shuffle*/, std::vector<std::vector<unsigned/*args position*/>> &results) 
    {
            ...
            for (unsigned i=0; i < numArgs; ++i)
            {
                if (visitedargs.insert(i).second)
                {
                    ...
                    if (sametypeargs.size() >= shuffle_size)
                    {
                        ...
                        getCombinations (sametypeargs, shuffle_size, 0, std::vector<unsigned>(), combinations);
  ...
  public:
    ...
    {
        llvm::errs() << "Unsuported yet: 'matchIRs' mathod of FunctionCall should not be called \n";
        assert (false);
    }
    ...
    {
        llvm::errs() << "Unsuported yet: 'prepareCloneIRs' mathod of FunctionCall should not be called \n";
        assert (false);
    }
            ...
            if (llvm::CallInst *call = llvm::dyn_cast<llvm::CallInst>(val))
            {
                ...
                for (auto &repl: mutationOp.getMutantReplacorsList())
                {
                    ...
                    else if (repl.getExpElemKey() == mNEWCALLEE)
                    {
                        ...
                        if (cf && cf->getName() == llvmMutationOp::getConstValueStr(repl.getOprdIndexList()[0]))    //'repl.getOprdIndexList()[0]' is it the function to match?
                        {
                    ...
                    else if (repl.getExpElemKey() == mSHUFFLE_ARGS)
                    {
                        ...
                        for (auto &argspos: combinations)
                        {
                            toMatchMutant.clear();
                            toMatchMutant.setToCloneStmtIROf(toMatch, MI);
                            llvm::CallInst *clonecall = llvm::dyn_cast<llvm::CallInst>(toMatchMutant.getIRAt(pos));
                    ...
                    {
                        assert (false && "Error: CALL should have only 'NEWCALLEE' or 'SHUFFLEARGS' as replacors");
                    }
                }
            }
        }
...
#endif //__KLEE_SEMU_GENMU_operatorClasses__FunctionCall__
